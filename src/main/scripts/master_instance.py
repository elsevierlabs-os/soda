...
#print(args.config, args.command)
#configs = load_config(args.config)
#print(configs)
#save_config(configs, args.config)


#print(verify_ami_exists(ec2, configs["IMAGE_ID"]))
#print(verify_instance_exists(ec2, configs["INSTANCE_ID"]))
#load(args.config)



#resp = ec2.describe_images(ImageIds=[configs["IMAGE_ID"]])
#print(">>>", len(resp["Images"]))
#print(">>>", resp["ResponseMetadata"]["HTTPStatusCode"])


#copy_pem_file(configs["KEY_FILE"])
#remove_pem_file(configs["KEY_FILE"])
...
#    tags = instance.meta.data["Tags"] 
#    name = get_tag_value(tags, "name")
#    owner = get_tag_value(tags, "owner")
#    print(instance.id, name, owner, instance.instance_type)
#    
### check to see if the instance is already running
##ami_id = "ami-cfe4b2b0"
##for instance in instances:
##    if instance.meta.data["ImageId"] == ami_id:
##        instance_id = instance.meta.data["InstanceId"]
##        tags = instance.meta.data["Tags"]
##        instance_name = get_tag_value(tags, "name")
##        print("EC2 instance {:s} ({:s}) based on AMI {:s} already running"
##              .format(instance_id, instance_name, ami_id))
#
### Example use:
##ssm_client = boto3.client("ssm") # Need your credentials here
##commands = ["""echo "hello world" """.rstrip()]
##instance_ids = ["i-096fffe55dcf9ba83"]
